...
> 逐条状态见 Phase 6.5 表。剩:状态点三态 / 图标下载缓存 / TLS 放行接线 / 批量解析深链 / CF 改写挂载。
>
...
| AutoLoader 播放期自动挂 | `danmaku_auto_loader.dart` | — | — | ❌ 编排 + 播放会话态,归宿主/前端 |
| 本地弹幕文件(xml/json/ass) | `danmaku_local_parser.dart` | — | — | ❌ 未做 |
...
> - **清除已完成**:只有逐条 `download_remove(id)`,无批量 → 页 11 顶栏按钮没后端。
> - **预取线程数/缓存上限写死**(代码里自承「PoC 暂固定 3 线程 / 1GB 封顶」)→ 设置项无效。
> - **CF 优选挂不上 choke point**:`cf_proxy_start` 起得来,但**没有 line 概念可改写**(见 Phase 1)。
...
| `activeLineUrl` 改写点(CF 优选的 choke point) | `:92 activeLineUrl` | `Account::direct_line_url()` | ◑ 上游地址已就位,**CF 反代改写还没挂上去** |
...
| 状态点三态(绿正常/黄需重登/灰未连) | `:154 authStateProvider` + `testConnection` | — | ❌ 现在只有 active 二态,且语义错 |
| 服务器图标(内置/网络/本地上传) | `:295 _materializeNetworkIcons` + `server_icon_cache.dart` | `Account.icon_url` 字段已有 | ◑ 字段在,**下载/缓存/本地上传未做** |
| 自签名 TLS 按 host 放行 | `:60 allowInsecureTls` `:324 _syncInsecureTlsHosts` | `Account.allow_insecure_tls` 字段已有 | ◑ 字段在,**HttpClient 放行逻辑未接** |
| 批量解析添加 + `linplayer://` 深链 | `server_batch_parser.dart` / `deep_link_service.dart`(341 行) | — | ❌ 页 06「批量解析导入」 |
...
| 恢复扫描判定 | `_needsRestore/_buildSearchQuery/_resolveCandidate` | `needs_restore`/`restore_search_query`/`pick_restore_candidate` | — | ✅ 判定已备,**HTTP 编排未接** |
| 跨服回传 | `writeback_service.propagate` 的选择段 | `writeback_targets` / `writeback_dedup_key` | — | ✅ 判定已备,**HTTP 编排未接** |
...
| 实时预读翻译 | `streaming_subtitle_translator.dart` | `StreamingTranslator::on_cue/warm/compose` | ✅ 逻辑在 core,**播放器控制归宿主** |
...
| Linux ffmpeg 自动下载 | `downloadFfmpeg` | — | ❌ 官方是 `.tar.xz`,缺 tar/xz2 依赖;返回明确错误让用户装包管理器 |

> **前置(Phase 2 表里标 ❌ 的两项)仍未做**:「字幕 cue 观测/内嵌字幕隐藏」+「loadLibassSubtitleMemory」。
> **没有它们,实时预读翻译接不上播放器** —— core 逻辑齐了,但这条链还差最后一环。整轨翻译不受影响。
>
